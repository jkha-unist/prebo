...
from pre_bo_truncated import pre_BO
import scipy
import math
...
pb.mo_coeff = mo_a1_full[pb.ao_mask, :]
...
mc_fci = mcscf.CASCI(mol_no_sym, no_cas, ne_cas)
mc_fci.fcisolver = csf_fci.csf_solver(mol_no_sym, smult=1)
ncsf_cut = 5
...
pb.get_V_csf()
pb.get_grad_ao()
# Using the specific calls from discrete_hff:
pb.get_td_D_csf(block_orth=False, finite_difference=True, dt=dt)
pb.get_dV_csf_2()
...
for istep in range(nstep + 1):
    ...
    pb.propagate_elec_expm(dt, l_tdnac=True)
