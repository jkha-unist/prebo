...
pb.get_V_csf()
pb.get_grad_ao()
# No need to align at t0, or if we do, it should be against a reference.
# The previous code was: pb.align_mo_coeff_spacewise(pb.mol, pb.mo_coeff)
# which is identity.
...
for istep in range(nstep + 1):
    ...
    pb.pos += dt * pb.vel
    # Enforce 1D motion along x-axis
    pb.pos[:, 1:] = 0.0
    pb.vel[:, 1:] = 0.0
